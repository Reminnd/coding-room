...
> Planning handoff: the JSON uses the existing stage-generic Actions grammar and remains unchanged. T05 remains present as the integrated recovery/dependency identity. The exact T05R00 Contract is `Accepted` with `confirmed_by_user=true` at the exact pushed Stage head containing this acceptance, but execution requires separate explicit user authorization of one `manual_pre_native_codex_exec` repair. T05R00 is deliberately absent from the Router tasks array and Ready Set. T05F00 remains `Accepted` with `confirmed_by_user=true`, but execution is blocked until T05R00 is integrated and a later fresh `run-once` is separately authorized. T05F01 remains `Proposed` with `confirmed_by_user=false`. T06 remains a `Planning Placeholder` and is not in the Router Ready Set.
...
- The exact T05R00 Contract is `Accepted` with `confirmed_by_user=true` at the exact pushed Stage head containing this acceptance, but execution requires separate explicit user authorization of one one-time `manual_pre_native_codex_exec` repair. T05R00 is not Router-dispatchable because the current native backend cannot create a thread. Local Bridge MUST NOT discover or execute it, native `run-once` MUST NOT execute it, and production MUST NOT add a native-to-`codex exec` fallback or compatibility mode.
