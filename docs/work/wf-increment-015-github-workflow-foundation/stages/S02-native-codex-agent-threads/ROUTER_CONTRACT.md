...
- Do not invoke `run-once` now. A separate fresh `run-once` authorization is still required, and its process MUST have started after T05R00 integration so it loads the repaired `codex-app-server.mjs`. That later `run-once` executes and integrates only T05F00 `-002`, then MUST stop.
