...
- The active T05F00 retry dispatch is `wf15-s02-t05f00-root-multi-agent-prompt-boundary-003`. Its exact Contract is `Proposed` with `confirmed_by_user=false`; it MUST be accepted again before cleanup or environment preparation and MUST NOT be dispatched now.
- After exact `-003` Contract acceptance, cleanup of the historical dirty `-002` Task worktree/local branch requires its separately ordered authorization. The Host MUST then create a fresh, clean `-003` Task branch/worktree at the exact current Stage and run exact `npm ci` from the repository lockfile before any `run-once`. This is Host execution-environment preparation, not T05F00 Coding, a Task deliverable, a Bridge fallback or a Task-owned file change. The production Bridge MUST NOT be changed to install dependencies automatically.
