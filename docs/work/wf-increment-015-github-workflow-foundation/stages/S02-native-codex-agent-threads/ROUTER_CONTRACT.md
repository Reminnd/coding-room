...
> Planning handoff: Router schema is unchanged; T05F00 dispatch identity rotated from `wf15-s02-t05f00-root-multi-agent-prompt-boundary-001` to `wf15-s02-t05f00-root-multi-agent-prompt-boundary-002`. T05 remains present as the integrated recovery/dependency identity. T05R00 is integrated from source `ba077fc1a39f85c179e65aa39b64646f4aed716a` to Stage commit `ad3e00989932828e58e742bce66a6cf1e8ab0745`. T05F00 Retry Contract is `Proposed` with `confirmed_by_user=false`; it must be separately accepted before any separately authorized fresh `run-once`. T05F01 remains `Proposed` with `confirmed_by_user=false`. T06 remains a `Planning Placeholder` and is not in the Router Ready Set.
...
- T05F00 Retry Contract is `Proposed` with `confirmed_by_user=false`. Its active dispatch is `wf15-s02-t05f00-root-multi-agent-prompt-boundary-002`; old dispatch `wf15-s02-t05f00-root-multi-agent-prompt-boundary-001` is a historical `needs_decision` event and MUST NOT be replayed or treated as current recovery state.
- Do not invoke `run-once` now. The user must first accept the exact retry Contract. After acceptance, a separate fresh `run-once` authorization is still required, and its process MUST have started after T05R00 integration so it loads the repaired `codex-app-server.mjs`. That later `run-once` executes and integrates only T05F00 `-002`, then MUST stop.
