...
The outer T05 Contract above remains Accepted and keeps the same goal, ownership, dispatch identity, Task branch, existing worktree, and original dispatch base. The internal multi-agent amendment generated from Stage SHA `740b51479421ab087323fc5ae66255a0d591d0a1` is pending separate user acceptance at the exact pushed Contract-bundle SHA; this planning commit does not authorize T05 resume or Coding.
...
5. Deliver only that Task's complete Contract bundle, dispatch envelope, owned paths and dependency facts as Root turn input. During the current S02 repair phase, the T05 Root may use native Codex multi-agent capability only through [`T05_MULTI_AGENT_ROUTER_CONTRACT.md`](./T05_MULTI_AGENT_ROUTER_CONTRACT.md) and [`T05_MULTI_AGENT_SUPERVISOR_ROUTER.md`](./T05_MULTI_AGENT_SUPERVISOR_ROUTER.md). Only Root may dispatch the exact A01–A04 subagents; child-spawned writing subagents remain forbidden.
...
10. When native thread creation, explicit `cwd`, requested model/effort, terminal event observation, or the accepted native internal multi-agent capability is unavailable, return `needs_decision`. Never fall back to `codex exec`, another provider, a shared thread, or a serial fake-agent execution.
...
execution:
  ...
  worker_spawned_subagents: t05_root_router_only
...
internal_orchestration:
  ...
  git_authority: none
  persistent_workflow_state: false
```

The protocol adapter and T05 internal file split remain inside the one logical T05 Task. Internal subagents are not Git Tasks and do not alter the Local Bridge DAG, Task branch/worktree ownership, GitHub lifecycle, or formal Review path. Do not add a generic executor/provider abstraction. The existing non-worker Supervisor Integration invocation retains its current independent gate; this Contract only replaces the Implementation Worker boundary.
...
- No child branch/worktree/commit/push/GitHub event and no serial fake-agent fallback. Root alone owns combined verification and one final amend of the existing T05 commit.
...
3. The T05 Root receives only the exact accepted Contract bundle/envelope/ownership/dependency facts. Root may dispatch only A01–A04 under the internal Router; every child receives only its own subcontract and cannot spawn a writing subagent.
...
## Native multi-agent internal repair amendment

```yaml
amendment_generation_source_stage_sha: 740b51479421ab087323fc5ae66255a0d591d0a1
exact_contract_bundle_acceptance: pending
resume_authorized_at_generation: false
outer_dispatch_id: wf15-s02-t05-native-codex-thread-backend-001
...
outer_task_identity_preserved: true
outer_worktree_count: 1
outer_task_branch_count: 1
child_git_authority: none
final_commit_count: 1
```

The accepted runtime gate is a later current-chat user decision that accepts the exact pushed Contract-bundle Stage SHA and authorizes T05 resume. Do not self-modify this historical generation-state field.
...
→ A04-cross-boundary-audit (read-only)
→ Root combined verification
→ Root amends the one existing T05 commit
→ existing Supervisor Integration gate
```

The subcontracts are:

- [`A01-app-server-transport`](./subtasks/A01-app-server-transport/TASK_CONTRACT.md)
- [`A02-capability-model-boundary`](./subtasks/A02-capability-model-boundary/TASK_CONTRACT.md)
- [`A03-coding-result-gate`](./subtasks/A03-coding-result-gate/TASK_CONTRACT.md)
- [`A04-cross-boundary-audit`](./subtasks/A04-cross-boundary-audit/TASK_CONTRACT.md)
...
`--stdio`, transport fallback, and backend fallback are forbidden. The existing model policy resolver resolves each policy against currently available models; neither this amendment nor a subcontract hard-codes a future concrete model version.

`tools/codex-github-bridge/supervisor.mjs` is not assigned to a child. Its current candidate change is preserved and A04 audits it read-only. If any further edit to it or another unowned path is necessary for Accepted T05 compliance, Root returns `needs_decision` instead of expanding scope.

Because this planning bundle is committed only to the Stage while the existing T05 candidate worktree must remain unchanged, the later resume dispatch MUST inject the complete accepted bundle into the Root turn. A path-only reference resolved solely inside the preserved candidate checkout is insufficient.
...
No implementation-time documentation update. This planning-only amendment and its internal Contracts are the complete authorized documentation change; T06 still depends on integrated T05 facts and owns the later exact authority/documentation synchronization.
...
- native multi-agent/subagent capability is unavailable, nested writing delegation is required, or A01/A02/A03 cannot run as the initial parallel Ready Set;
- `tools/codex-github-bridge/supervisor.mjs` or another unowned path requires a further change.
