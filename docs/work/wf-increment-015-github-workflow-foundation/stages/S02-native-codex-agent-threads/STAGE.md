...
- status: `waiting_for_T05R00_execution_authorization`
...
- bootstrap_repair: [`T05R00-native-sandbox-wire-mode`](./tasks/T05R00-native-sandbox-wire-mode/TASK_CONTRACT.md); `Accepted`, `confirmed_by_user=true`, `execution_authorized=false`, `execution_surface=manual_pre_native_codex_exec`; not Router-dispatchable
...
- lifecycle: `waiting_for_T05R00_execution_authorization`
- integration_facts: T05 is `integrated`; T05R00 is `Accepted` with `confirmed_by_user=true`, `execution_authorized=false`, `execution_surface=manual_pre_native_codex_exec` and `manual_pre_native_repair=true`; T05F00 remains `Accepted` with `confirmed_by_user=true` but is blocked by T05R00; T05F01 is `Proposed` with `confirmed_by_user=false` and `execution_authorized=false`; T06 remains a `Planning Placeholder` with `dispatchable=false`
...
→ T05R00 exact Contract Accepted and confirmed_by_user=true at the exact pushed Stage head containing this acceptance
...
→ T05R00 exact Contract is Accepted and confirmed_by_user=true at the exact pushed Stage head containing this acceptance
