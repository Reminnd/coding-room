...
- status: `waiting_for_T05F00_retry_003_environment_preparation`
...
The active retry dispatch is `wf15-s02-t05f00-root-multi-agent-prompt-boundary-003`; its exact Contract is `Accepted` with `confirmed_by_user=true`. The current process stops before separately authorized cleanup and fresh worktree creation. The Host must run exact `npm ci` in that fresh, clean, exact-current-Stage worktree, then verify unchanged package manifests, clean Git state, ignored-only `node_modules` and resolvable TypeScript. This is Host execution-environment preparation, not T05F00 Coding, a Task deliverable, a Bridge fallback or a Task-owned change. Environment preparation is not complete, and only after it passes may the user separately authorize a fresh `run-once`; no fresh `run-once` is currently authorized. The future executing Bridge process will still use the one-time legacy transition envelope. After T05F00 integration, that process MUST stop.
