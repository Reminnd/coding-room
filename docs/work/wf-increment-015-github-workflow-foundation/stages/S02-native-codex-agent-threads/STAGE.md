...
- status: `waiting_for_T05F00_retry_contract_acceptance`
...
- retry_proposed_task: [`T05F00-root-multi-agent-prompt-boundary`](./tasks/T05F00-root-multi-agent-prompt-boundary/TASK_CONTRACT.md); `Proposed`, `confirmed_by_user=false`, active dispatch `wf15-s02-t05f00-root-multi-agent-prompt-boundary-002`; the exact retry Contract must be separately accepted before any fresh `run-once` authorization
...
→ user separately accepts the exact retry Contract
...
→ user separately accepts that exact retry Contract
→ user separately authorizes a fresh run-once
