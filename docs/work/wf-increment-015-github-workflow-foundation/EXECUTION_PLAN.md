...
- status: `s03cf01_repaired_locally_waiting_for_stage_push_authorization`
...
S03 Stage 与 Draft PR #7 已建立，当前 remote candidate `ddbc5a35d734eaca908090013b3ce29202086483` 未被接受。当前 gate 顺序固定为：local `S03CF01` repair → 单独 repair Stage push authorization → existing `stage/**` Actions Router validation/exact handoff → user acceptance of exact pushed repaired Contract SHA → separate T06 environment/`run-once` authorization → native Worker → Controller candidate commit → Supervisor Integration → Stage verification → fixed Chat Formal Review → exact-SHA user acceptance → non-force fast-forward。当前 repair 不跨越任何后续 gate。

Repository 已 Ready；不得再次 bootstrap。Normal `start`/`run-once` 继续只做 read-only prerequisite checks，不得 silent bootstrap、fallback、replay S02 或把尚未 push、验证和接受的 local repair commit 当作 dispatch authority。
