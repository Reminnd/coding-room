...
本ADR继续标记为`Proposed`：该设计已由Increment 10实现、Review、用户最终接受并由本次提交进入版本化`main`，但尚未完成v0.4 runtime/database/binding cutover，因而不是Current operational capability。版本化不等于runtime cutover或旧database处理授权。
