...
The Room itself stays in the planning-only states `DISCUSSION` / `ARCHITECTURE_REVIEW` / `WAITING_FOR_USER_CONFIRMATION`. Execution state lives in per-Run work items: each Run has its own ready/execution/needs-decision/review/accepted lifecycle, its own worktree lease, baseline and session lineage. The snapshot's `run_work_items` list (sorted by `created_at` then `run_id`) is the authority for which Run waits on which actor; the Skill never infers a single "current Run".
...
2. **Attempt identity**: choose a fresh non-empty `attempt_id` that does not exist in this Room (per attempt, per invocation). The persisted Run owns the worktree baseline (frozen by the first attempt's clean-Git gate) and the session lineage — the command never carries a `--task-id` or `--baseline-head`.
